...
def parse_args():
    ...
    parser.add_argument(
        "-c", "--count", help="Provide raw k-mer raw counts, not normalized [False]", action="store_true", default=False
    )
    parser.add_argument(
        "-f",
        "--frac",
        help="Provide k-mer counts normalized by total number of k-mers [False]",
        action="store_true",
        default=False,
    )
...
def launch_seq_kmers_pool(fastx, ftype, k, threads, target_range, combined_kmers, count, frac, zipped):
    ...
    if ftype == "fastq":
        for read_num, (read_id, seq, qual) in enumerate(FastqGeneralIterator(fast_file)):
            args, status = build_args_for_kmer_calc(
                read_num, target_range, args, read_id, seq, k, combined_kmers, lengths_d, count, frac
            )
            if status == "over":
                break
