...
def seqmatch_records(path, top_n=TOP_N_HITS):
    ...
    for rank, (_, r) in enumerate(df.head(top_n).iterrows(), start=1):
        species = r.get(0, "")
        if _is_unclassified(species):
            records.append({"rank": rank, "species": "unclassified", "taxid": "", "s_ab_score": "", "selected": False})
            continue
        records.append(
            {"rank": rank, "species": species, "taxid": r.get(1, ""), "s_ab_score": r.get(2, ""), "selected": rank == 1}
        )
    return records
...
def kraken2_records(path):
    ...
    if assigned.empty:
        return [{"rank": 1, "species": "unclassified", "taxid": "", "lca_reads_pct": "", "selected": False}]
    row = assigned.iloc[-1]
...
def parse_blast_hits(path, top_n=TOP_N_HITS):
    ...
    rows = [
        {"Species": _mark(r), "% Identity": _blank(r["pct_identity"]), "Bitscore": _blank(r["bitscore"])}
        for r in blast_records(path, top_n)
    ]
...
def parse_seqmatch_hits(path, top_n=TOP_N_HITS):
    ...
    rows = [{"Species": _mark(r), "S_ab score": _blank(r["s_ab_score"])} for r in seqmatch_records(path, top_n)]
...
def parse_kraken2_hit(path):
    ...
    rows = [{"Species (LCA)": _mark(r), "Reads %": _blank(r["lca_reads_pct"])} for r in kraken2_records(path)]
    ...
    if barcode == "discontinued":
        ...
        if relevant_rows.empty:
            raise SystemExit(
                ...
                "Barcodes present: {2}".format(barcode, file, ", ".join(map(str, info["Barcode"].tolist())))
            )
...
def assess_cluster(recs, winner_token, neg_keys):
    ...
    failed = [
        ...
        if _rank1(recs.get(t, [])) is None or _is_unclassified(_rank1(recs.get(t, []))["species"])
    ]
    if failed:
        agreement = "red"
...
def build_qc_html(clusters, cluster_info, neg_species, min_abundance=SHOW_MIN_ABUNDANCE, racon_failed=frozenset()):
    ...
    for cid in shown:
        ...
        reads_txt = "{0} ({1}%)".format(reads, pct) if str(reads).strip() not in ("", "nan") else "-"
    ...
    legend = (
        '<p class="qc-legend">Per-cluster checks &mdash; '
    ...
    if unshown:
        ...
        footer = (
            '<p class="qc-legend">There were {0} additional cluster(s) which are not shown '
            "here (each below the {1:.0f}% abundance threshold). The largest was {2:.1f}% "
            "abundance. The full results are in the CSV file.</p>".format(len(unshown), min_abundance, largest)
        )
    ...
    if racon:
        footer += (
            '<p class="reason a"><span class="lamp"></span><b>Unpolished consensus.</b> '
...
def _build_detail(cid, anchor, recs, a, lights):
    ...
    if not failed and lights["agreement"] != "green":
        ...
        reasons.append((lights["agreement"], "<b>Classifiers disagree.</b> " + "; ".join(calls)))
    ...
    for token, label in (("blast", "BLAST"), ("seqmatch", "SeqMatch"), ("kraken2", "kraken2")):
        ...
        if not real:  # a failed/unclassified classifier is shown explicitly, not hidden
            mini_rows.append(
                "<tr><td>{lab}</td><td><i>no classification</i></td>" '<td class="c">-</td></tr>'.format(lab=label)
            )
            continue
        for i, r in enumerate(real):
            ...
            mini_rows.append(
                ...
                '<td class="c">{sc}</td></tr>'.format(lab=label if i == 0 else "", sp=_esc(r["species"]), sc=score)
            )
    mini = (
        "<table><thead><tr><th>Classifier</th><th>Hit</th>"
        '<th class="c">Score</th></tr></thead><tbody>{0}</tbody></table>'.format("".join(mini_rows))
        if mini_rows
        else ""
    )

    return '<tr class="qc-detail" id="{a}"><td colspan="7">' "<b>Cluster {c}</b>{reasons}{mini}</td></tr>".format(
...
def load_all_clusters(cluster_logs_dir):
    ...
    """
    clusters = {}
    if not cluster_logs_dir or cluster_logs_dir in ("none", "unknown") or not os.path.isdir(cluster_logs_dir):
        return clusters
...
def build_dropped_clusters_html(cluster_logs_dir, classified_ids, min_pct=SHOW_MIN_ABUNDANCE):
    ...
    rows = "".join(
        '<tr><td>{0}</td><td class="c">{1} ({2:.1f}%)</td></tr>'.format(_esc(cid), reads, reads / total * 100.0)
        for cid, reads in sorted(dropped.items(), key=lambda kv: -kv[1])
    )
    table = (
        '<table><thead><tr><th>Cluster</th><th class="c">Reads (%)</th></tr></thead>'
        "<tbody>" + rows + "</tbody></table>"
    )
    note = (
        '<p class="reason ' + _map3(level) + '"><span class="lamp"></span>'
        ...
        "method cannot identify these; treat a large unidentified fraction with care.</p>".format(
            n=len(dropped), r=dropped_reads, p=dropped_pct
        )
    )
...
def parse_args():
    ...
    parser.add_argument("--demux", default="unknown", help="demultiplexing method and software version")
    parser.add_argument("--clustering_size", default="unknown", help="Amount of reads used for UMAP HDBSCAN clustering")
    parser.add_argument("--positive", default="unknown", help="File path for positive control results")
    parser.add_argument("--negative", default="unknown", help="File path for negative control results")
    parser.add_argument("--reads_count", default="0", help="Reads count after quality control")
    parser.add_argument("--kit", default="unknown", help="Kit used for barcoding and demultiplexing")
    parser.add_argument("--report_template", help="path to report template")
    parser.add_argument("--logo", help="custom logo")
    parser.add_argument("--run_id", default="unknown", help="Run ID of the sequencing run")
    parser.add_argument("--seq_start", default="unknown", help="Start time of the sequencing run")
    parser.add_argument(
        "--hit_details",
        default="none",
    ...
    parser.add_argument("--blast_db_name", default="n/a", help="folder name of the BLAST database used")
    parser.add_argument("--kraken2_db_name", default="n/a", help="folder name of the kraken2 database used")
    parser.add_argument("--seqmatch_db_name", default="n/a", help="folder name of the SeqMatch database used")
    parser.add_argument("--taxonomy_name", default="n/a", help="file name of the taxonomy dump used")
    parser.add_argument(
        "--cluster_logs",
        default="none",
...
def _plain_table(df, classes=None):
    ...
    if classes:
        html = html.replace('class="dataframe"', 'class="dataframe {0}"'.format(classes), 1)
    return html
...
def _lookup_meta(metadata_table, label_substr, default="unknown"):
    ...
    try:
        m = metadata_table[
            metadata_table["Metadata"].astype(str).str.contains(label_substr, case=False, na=False, regex=False)
        ]
        if len(m):
...
def main(args):
    ...
    if args.barcode == "discontinued":
        ...
        if assay_type == "16S":
            database_info = (
                "16s bacterial sequencing results were compared against 16S & 18S database, build 18 Jan 2022."
            )
        ...
        **POSITIVE CONTROL**
        ...
        run_params = pd.DataFrame(
            list(
                zip(
                    [
                        "Run ID: " + str(run_id),
                        "Barcoding kit: " + str(barcoding_kit),
                        "Demultiplex method: " + str(demux_method),
                    ],
                    [
                        "Species database: " + str(species_database),
                        "Clustering size: " + str(clustering_size),
                        "Sample barcode: " + str(metadata_table.iloc[1, 1]),
                    ],
                )
            ),
            columns=["GridION properties", "NanoPATH properties"],
        )
        ...
        db_rows = [
            ("BLAST", args.blast_db_name),
            ("kraken2", args.kraken2_db_name),
            ("SeqMatch", args.seqmatch_db_name),
