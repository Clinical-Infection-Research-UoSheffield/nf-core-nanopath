...
def parse_args():
    ...
    parser.add_argument(
        "--umap_n_neighbors",
        help="The size of the local neighborhood UMAP will look at when attempting to learn the manifold structure of the data [15]",
        type=int,
        default=15,
    )
    parser.add_argument(
        "--umap_min_dist",
        help="The minimum distance apart that points are allowed to be in the low dimensional representation. [0.1]",
        type=float,
        default=0.1,
    )
    parser.add_argument(
        "--min_samples", help="Measure of how conservative the clustering should be. [None]", type=str, default="n"
    )
    parser.add_argument(
        "--min_cluster_size", help="Minimum number of reads to call a independent cluster [50]", type=int, default=50
    )
    parser.add_argument(
        "--cluster_sel_epsilon", help="Minimun distance to separate clusters. [0.5]", type=float, default=0.5
    )
...
def main(args):
    ...
    X_embedded = umap.UMAP(
        n_neighbors=int(args.umap_n_neighbors), min_dist=int(args.umap_min_dist), verbose=2
    ).fit_transform(X)
    ...
    umap_out["bin_id"] = hdbscan.HDBSCAN(
        ...
        cluster_selection_epsilon=int(args.cluster_sel_epsilon),
    ...
    plt.title(
        "Projecting "
        + str(len(umap_out["bin_id"]))
        + " reads. "
        + str(len(umap_out["bin_id"].unique()))
        + " clusters generated by HDBSCAN",
        fontsize=18,
    )
